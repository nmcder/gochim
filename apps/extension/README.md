...
남의 페이지가 어떤 CSS를 쓸지 모르므로, 스모크 테스트에 **적대적 CSS** 구간을 두었습니다 —
21px 세리프, 자간 2.5px, 어간 6px, 행간 2.4, 비대칭 여백, 6px 점선 테두리.

![적대적 CSS에서의 거울 레이어 정렬](../../docs/images/extension-hostile-css.png)
...
품사 기반 판정(3층)에는 WASM 0.4MB + 모델 1.2MB가 필요합니다.
이걸 콘텐츠 스크립트에 넣으면 맞춤법을 쓰지도 않을 페이지까지 전부 1.6MB를 지고 시작합니다.

그래서 **설정에서 켠 사람에게만, 처음 필요한 순간에** 별도 스레드로 올립니다.

```
입력 → 1층(문자열) 즉시 실행 → 밑줄 표시
           ↓ postMessage
        Worker: 형태소 분석 → 3층 결과 → 합쳐서 다시 그림
```

사용자 입장에서는 밑줄이 즉시 뜨고, 잠시 뒤 몇 개가 더 붙습니다.
워커는 **마지막 요청의 결과만** 반영합니다 — 타이핑 중에 앞선 요청은 이미 쓸모없습니다.
분석기를 못 불러와도 1층은 그대로 돌아갑니다.
