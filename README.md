...
| 재현율 | 0.952 (1층만) → **0.957** (형태소 층 포함) |
| 규칙 | 문자열 199개 + 형태소 5개. 예시 839개, 반례 1,251개가 테스트로 강제됨 |
| 테스트 | 3,247개 |
...
| | |
| --- | --- |
| 표본 | 15편 9,613자 · 오류 **464건** |
| 재현율 | 0.677 (1층만) → **0.821** (형태소 층 포함) · 과교정 **0건** |
...
(1층과 형태소 층 각각), 규칙 199개의 예시 자가검증, 정답 불변식, 재현율 하한선.
