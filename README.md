...
| 규칙 | 문자열 187개 + 형태소 2개. 예시 714개, 반례 1,006개가 테스트로 강제됨 |
| 테스트 | 2,812개 |
...
확인하려고 **갈래가 다른 글 일곱 편**을 따로 만들었다 — 메신저·이메일·자기소개서·댓글·
후기·리포트·일기. 정답은 쓴 쪽과 다른 눈이 반증 검증했다.

| | |
| --- | --- |
| 표본 | 7편 3,940자 · 오류 **223건** |
| 재현율 | **0.901** · 과교정 **0건** |

처음 넣었을 때는 **0.435**였다. 일기에 없던 오류는 규칙 자체가 없었기 때문이다.
갈래별로는 댓글·후기 0.93, 이메일·리포트 0.85 언저리다.
`npm run corpus:report` 로 갈래별·분류별 성적과 못 잡은 목록을 본다.

`npm run guard` 는 넘으면 안 되는 선을 한 번에 확인한다 — 정상 문장 691개 오탐 0,
규칙 187개의 예시 자가검증, 정답 불변식, 재현율 하한선. 어기면 종료 코드 1이다.
...
| | |
| --- | --- |
| 표본 | 일기 1편 1,455자 · 오류 **63건** (3단계 검증한 정답) |
| 재현율 | **1.000** · 과교정 0건 |
