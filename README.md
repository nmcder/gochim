...
| 재현율 | 0.944 (1층만) → **0.952** (형태소 층 포함) |
| 규칙 | 문자열 118개 + 형태소 2개. 예시 356개, 반례 320개가 테스트로 강제됨 |
| 테스트 | 1,095개 |
...
저장소 안의 한국어 산문 4,707줄(README·ADR·규칙 파일 주석)에도 규칙을 돌려 봅니다.
지적이 나오는 자리는 규칙 파일에 일부러 넣어 둔 오류 예시뿐입니다.
...
| 재현율 | **0.937** · 과교정 0건 |
| 모두 고친 뒤 정답과 **글자까지 같은 문단** | **5/7** · 나머지 문단의 어절 일치 0.924 |
