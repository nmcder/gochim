...
export function josaOf(word: string, pair: readonly [string, string]): string {
 ...
 */
export function adaptJosa(from: string, to: string, rest: string): { consumed: number; josa: string } | null {
  for (const pair of JOSA_PAIRS) {
    const current = josaOf(from, pair)
    if (!rest.startsWith(current)) continue
    const adapted = josaOf(to, pair)
    if (adapted === current) return null
