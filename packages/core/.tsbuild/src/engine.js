...
function resolveOverlaps(found) {
    ...
    for (const d of sorted) {
        ...
        const better = d.confidence > other.confidence ||
 ...
 */
export function check(text, options = {}, defaultRules = [], defaultMorphRules = []) {
    if (!text)
        return [];
    ...
    const accept = (d) => {
        ...
        if (d.confidence < minConfidence)
            return;
        if (overlapsProtected(protectedRs, d.start, d.end))
            return;
        if (ignore?.has(ignoreKey(d)))
            return;
        found.push(d);
    };
    // ── 1층: 문자열 규칙 ────────────────────────────────────────
    for (const rule of rules) {
        if (categories && !categories.has(rule.category))
            continue;
...
export function mergeDiagnostics(...lists) {
    return resolveOverlaps(lists.flat());
}
/**
 * 진단을 원문에 적용한다.
 *
 ...
 */
export function applyFixes(text, diagnostics, pick = (d) => d.suggestions[0] ?? null) {
