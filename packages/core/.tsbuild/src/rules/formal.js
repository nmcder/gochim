...
export const nnbDeung = defineRule({
    ...
    resolve(ctx) {
        ...
        if (WORD_DEUNG.has(head.slice(-1) + '등') || WORD_DEUNG.has(head.slice(-2) + '등'))
            return null;
        // 나열의 '등'은 앞에 쉼표가 있다. 이 조건이 없으면 '평등을'류를 전부 훑는다.
        if (!/[,·]/.test(ctx.text.slice(0, ctx.index)))
            return null;
        return {
...
export const kkeseoAgreement = defineRule({
    ...
    resolve(ctx) {
        ...
        if (/제가|저는|저희|이가|가\s|은\s|는\s|니까|테니|어서|아서|으면/.test(middle))
            return null;
        ...
        return {
            ...
            explain: "주격 조사 '께서'는 주어를 높이는 형태입니다. 서술어에 '-시-'를 넣지 않으면 높임이 반쪽만 됩니다.",
            refs: ['표준 언어 예절'],
        };
    },
