...
export const nnbSu = defineRule({
    ...
    resolve(ctx) {
        ...
        if (!hasL(prev))
            return null;
        if (NOUN_SU.has(prev + '수'))
            return null;
        return {
            suggestions: [`${prev} 수 ${tail}`],
            message: "의존명사 '수'는 앞말과 띄어 씁니다.",
            explain: "가능성·능력을 뜻하는 '수'는 의존명사입니다. 관형사형 어미 '-ㄹ' 뒤에서 띄어 쓰고, 뒤의 '있다/없다'도 별개의 용언이라 또 띄어 씁니다.",
            refs: ['한글 맞춤법 제42항'],
        };
    },
    examples: [
...
export const nnbSuBakke = defineRule({
    ...
    resolve(ctx) {
        ...
        if (!hasL(prev) || NOUN_SU.has(prev + '수'))
            return null;
        return {
...
export const nnbBa = defineRule({
    ...
    resolve(ctx) {
        const [, prev = '', josaCh = ''] = ctx.match;
        if (finalOf(prev) !== 'ㄴ' && prev !== '는' && prev !== '던')
            return null;
        return {
...
export const nnbJung = defineRule({
    ...
    resolve(ctx) {
        ...
        return {
            ...
            explain: "'회의 중, 근무 중'처럼 띄어 씁니다. 다만 '부재중·한밤중·그중'은 한 단어라 붙여 쓰므로, 이 규칙은 확인된 말에만 적용합니다.",
            refs: ['한글 맞춤법 제42항'],
        };
    },
