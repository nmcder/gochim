...
export const eomiGe = defineRule({
    ...
    resolve(ctx) {
        ...
        if (finalOf(prev) !== 'ㄹ')
            return null;
        if (HONORIFIC_L.has(prev) || TIME_L.has(prev))
            return null;
        return {
            suggestions: ['게'],
            offset: 1,
            ...
            explain: "'-ㄹ게'는 [께]로 소리 나지만 된소리로 적지 않는 어미입니다. '-ㄹ까/-ㄹ꼬'처럼 의문을 나타내는 어미만 된소리로 적습니다.",
            refs: ['한글 맞춤법 제53항'],
        };
    },
    examples: [
...
export const endingRules = [eomiGe, eomiGeol, seosulYeyo, seosulIeyo];
