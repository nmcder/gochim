...
const isJosa = (pos) => pos.startsWith('J');
...
function splitFinding(word, at, nnb) {
    ...
    return {
        ...
        explain: '형태소 분석 결과 이 자리의 말이 의존명사로 쓰였습니다. 의존명사는 앞말과 띄어 씁니다. (같은 글자라도 조사로 쓰이면 붙여 씁니다)',
        refs: ['한글 맞춤법 제42항'],
    };
}
export const morphNnbSpacing = {
    ...
    run(ctx) {
        ...
        for (const word of ctx.words) {
            ...
            if (word.morphemes[index - 1]?.pos === 'SN' || /^\d/.test(word.text))
                continue;
...
export const morphJosaAttach = {
    ...
    run(ctx) {
        ...
        for (let i = 1; i < ctx.words.length; i += 1) {
            ...
            if (word.morphemes.length === 0 || !word.morphemes.every((m) => isJosa(m.pos)))
                continue;
            ...
            if (word.text.length < 2)
                continue;
            if (RISKY_JOSA.has(word.text))
                continue;
            // '보다'는 뒤에 정도부사가 올 때만 비교의 조사다.
            if (word.text === '보다' && !BODA_OK.test(ctx.text.slice(word.end)))
                continue;
            // 조사 '밖에'는 반드시 부정 표현과 호응한다. 아니면 바깥을 뜻하는 명사 '밖'이다.
            if (word.text === '밖에' && !/^\s*(없|모르|못|안[\s가-힣])/.test(ctx.text.slice(word.end)))
                continue;
            found.push({
                start: prev.start,
                end: word.end,
...
export const allMorphRules = [morphNnbSpacing, morphJosaAttach];
