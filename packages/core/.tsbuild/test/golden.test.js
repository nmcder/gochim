...
function score() {
    const result = { truePositives: 0, falsePositives: [], falseNegatives: [], exactFixes: 0 };
    for (const testCase of golden.cases) {
        ...
        for (const d of diagnostics) {
            ...
            if (index === -1)
                result.falsePositives.push({ sentence: testCase.wrong, diagnostic: d });
            else {
        ...
        testCase.spans.forEach((span, index) => {
            if (!covered.has(index))
                result.falseNegatives.push({ id: testCase.id, sentence: testCase.wrong, span });
        });
        if (fix(testCase.wrong) === testCase.right)
            result.exactFixes += 1;
    }
    ...
    for (const sentence of [...new Set(golden.cases.map((c) => c.right)), ...golden.negatives.map((n) => n.text)]) {
        for (const d of check(sentence))
            result.falsePositives.push({ sentence, diagnostic: d });
    }
    return result;
}
const s = score();
...
describe(`골든 테스트셋 (cases ${golden.cases.length} · negatives ${golden.negatives.length})`, () => {
    ...
    it('오탐 유도 문장(negatives)에도 진단이 나오지 않는다', () => {
        const offenders = golden.negatives.flatMap((n) => check(n.text).map((d) => `${d.ruleId}: "${d.text}" ← ${n.text}\n    함정: ${n.trap}`));
        expect(offenders).toEqual([]);
    });
