...
describe('check', () => {
  ...
  it('겹치는 진단은 하나만 남는다', () => {
    ...
    const overlapping = diagnostics.filter((a, i) =>
      diagnostics.some((b, j) => i !== j && a.start < b.end && b.start < a.end),
    )
...
describe('규칙 목록', () => {
  ...
  it('모든 규칙은 전역 정규식을 쓴다', () => {
    for (const rule of allRules) expect(rule.pattern.flags).toContain('g')
  })
