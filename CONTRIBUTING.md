...
새 규칙은 기본적으로 **밑줄만 긋습니다.** 자동 고침이 그 규칙을 쓰려면
`autoFixSafe: true`를 손으로 켜야 하고, 켜려면 조건 셋을 전부 만족해야 합니다.

```ts
export const myRule = defineRule({
  id: 'my-rule',
  autoFixSafe: true,   // ← 조건을 갖췄을 때만
  ...
})
```
...
| 조건 | 왜 |
| --- | --- |
| 밖에서 온 정상 글에서 **발화 0건** | `wild.json` 126문장과 저장소 자기 산문에서 한 번이라도 뜨면 그것은 오탐입니다 |
| 표본에서 **실제 오류를 잡은 적이 있다** | "조용해서 안 걸린 것"과 "일하는 걸 본 것"은 다릅니다. 아무 데서도 안 뜬 규칙은 근거가 없는 것이지 안전한 것이 아닙니다 |
| **반례가 하나 이상** | 규칙을 지은 사람이 "이건 건드리면 안 된다"를 적어 두었다는 증거입니다 |

`npm run guard`가 **선언을 검증합니다.** 조건을 안 갖췄는데 켜 두면 종료 코드 1입니다.
반대 방향은 강제하지 않습니다 — 숫자가 통과해도 위험한 줄 안다면 꺼 두는 것이 맞습니다.

### 왜 `severity`로 정하지 않는가

`severity`는 **"이것이 틀렸는가"라는 국어적 판정**이고, `autoFixSafe`는
**"사람이 보지 않아도 이 규칙을 믿을 수 있는가"라는 공학적 판정**입니다.

둘은 다른 축인데 예전에는 겹쳐 썼습니다. 그 결과 국어적으로는 분명히 옳은 규칙이,
자기 **가드**가 뚫리는 바람에 맞는 글을 묻지 않고 고쳐 버렸습니다.
`morph-eojeol-split`이 그 자리입니다 — 실문 재현율의 30%를 지고 있어 끌 수 없지만,
분석기가 모르는 합성어를 그럴듯하게 쪼개므로 자동 적용은 안 됩니다.
**밑줄로는 남고 자동 적용에서만 빠집니다.**

경고(`severity: 'warning'`)는 어느 경우에도 자동 적용되지 않습니다.
그 조건은 엔진이 `autoFixSafe` 안에 합쳐 두었으니 따로 신경 쓸 것 없습니다.
